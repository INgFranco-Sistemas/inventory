...
<template>
    <div>
        <VCard title="🏪 Sucursales">
            <VCardText>
                <VRow class="justify-space-between">
                    <VCol cols="3">
                        <VTextField Placeholder="Search Sucursal" density="compact" class="me-3" v-model="searchQuery"
                            @keyup.enter="list" />
                    </VCol>
            ...
            <VDataTable :headers="headers" :items="list_sucursales" :items-per-page="5" class="text-no-wrap">
        ...
        <SucursalEditDialog v-if="sucursal_selected_edit && isSucursalEditDialogVisible"
            v-model:isDialogVisible="isSucursalEditDialogVisible" :sucursalSelected="sucursal_selected_edit"
            @editSucursal="addEditSucursal"></SucursalEditDialog>
        <SucursalDeleteDialog v-if="sucursal_selected_delete && isSucursalDeleteDialogVisible"
            v-model:isDialogVisible="isSucursalDeleteDialogVisible" :sucursalSelected="sucursal_selected_delete"
            @deleteSucursal="addDeleteSucursal"></SucursalDeleteDialog>
    </div>
</template>
