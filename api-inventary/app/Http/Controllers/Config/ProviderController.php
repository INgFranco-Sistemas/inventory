...
class ProviderController extends Controller
{
    ...
    {
        $is_provider_exists = Provider::where("ruc",$request->title)->first();
        if($is_provider_exists){
            return response()->json([
                "message" => 403,
                "message_text" => "EL PROVEEDOR YA EXISTE, INTENTE UN RUC DIFERENTE"
            ]);
        }
    ...
    {
        $is_provider_exists = Provider::where("ruc",$request->title)->where("id","<>",$id)->first();
        if($is_provider_exists){
            return response()->json([
                "message" => 403,
                "message_text" => "EL PROVEEDOR YA EXISTE, INTENTE UN RUC DIFERENTE"
